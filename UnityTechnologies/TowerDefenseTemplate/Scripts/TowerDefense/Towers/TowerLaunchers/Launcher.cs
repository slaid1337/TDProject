...
namespace TowerDefense.Towers.TowerLaunchers
{
	public abstract class Launcher : MonoBehaviour, ILauncher
	{
		...
		public void PlayVFX(VisualEffect visualEffect, Vector3 origin, Vector3 lookPosition)
		{
			if (visualEffect == null)
			{
				return;
			}
			visualEffect.transform.position = origin;
			visualEffect.transform.LookAt(lookPosition);
			visualEffect.Play();
		}
